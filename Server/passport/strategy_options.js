...
passport.use(
  ...
  new Strategy(options, async (req, email, password, cb) => {
    try {
      const existsEmail = await db.User_Mobile.findFirst({
        where: { email },
      });
      ...
      const notifications = await db.Notifications_Mobile.create({
        data: {
      ...
      const user = await db.User_Mobile.create({
        data: {
          email: email,
...
passport.use(
  ...
  new Strategy(options, async (email, password, cb) => {
    try {
      const user = await db.User_Mobile.findFirst({
        where: { email },
        include: {
          Notifications: true,
        },
