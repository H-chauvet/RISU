...
router.get('/', async (req, res) => {
  ...
  try {
    ...
    if (note != null) {
      if (note < 0 || note > 5) {
        return res.status(401).send(res.__('invalidRating'))
      }
    }
  ...
  passport.authenticate('jwt', { session: false }), async (req, res) => {
    try {
      ...
      if (!req.body.note || req.body.note === '') {
        return res.status(401).send(res.__('missingRating'))
      }
      if (req.query.itemId == null) {
        return res.status(401).send(res.__('missingItemId'))
      }
...
router.delete('/:opinionId', jwtMiddleware.refreshTokenMiddleware,
  passport.authenticate('jwt', { session: false }), async (req, res) => {
    try {
      ...
      if (!opinion) {
        return res.status(401).send(res.__('reviewNotFound'))
      }
      ...
      return res.status(201).send(res.__('reviewDeleted'))
    } catch (err) {
      console.error(err.message)
      return res.status(401).ssend(res.__('errorOccured'))
    }
  }
)
