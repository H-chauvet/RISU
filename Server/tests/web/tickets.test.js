...
describe("Ticket Routes Tests", () => {
  ...
  it("get some tickets", async () => {
    const mockedTickets = [{
        id: 1, content: "Test1", title: "Test", creatorId: "123", createdAt: Date.now.toString(), assignedId : "1234", chatUid: "1", closed: false
    }, {
        id: 2, content: "Test2", title: "Test", creatorId: "123", createdAt: Date.now.toString(), assignedId : "1234", chatUid: "1", closed: false
    }
    ]
  ...
  it("should get user tickets", async () => {
    ...
    const mockedTickets = [{
        id: 1, content: "Test1", title: "Test", creatorId: "test-uuid", createdAt: Date.now.toString(), assignedId : "1234", chatUid: "1", closed: false
    }, {
        id: 2, content: "Test2", title: "Test", creatorId: "123", createdAt: Date.now.toString(), assignedId : "test-uuid", chatUid: "1", closed: false
    }
    ]
    ticketCtrl.getAllUserTickets.mockResolvedValueOnce(mockedTickets)

    const response = await supertest(app).get("/user-ticket/test-uuid")
    .set("Authorization", "Bearer mockedAccessToken");
  ...
  it("should assign some tickets", async () => {
    ...
    ticketCtrl.assignTicket.mockResolvedValueOnce(
        "Mocked message response",
    );
    ...
    const response = await supertest(app).put("/assign/test-uuid")
    .send(requestBody)
    .set("Authorization", "Bearer mockedAccessToken");
